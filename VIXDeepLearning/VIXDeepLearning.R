...
log_df <- df %>% arrange(DATETIME) %>% group_by(DATE) %>% 
    mutate_at(vars(-DATETIME, -DATE), .funs = function(x) log(x/lag(x))) %>% 
    ungroup() %>% select(DATETIME, DATE, everything()) %>% mutate(VIX_PRED = lead(VIX)) %>% drop_na()
...
model %>% layer_lstm(units = 50, input_shape = c(1, 10), batch_size = 10, return_sequences = FALSE, 
    activation = "tanh", stateful = TRUE, dropout = 0.2) %>% 
...
# Train the model
model %>% fit(x_train_lstm, y_train_lstm, batch_size = 10, epochs = 10, verbose = 2, 
...
vix_pred <- df %>% filter(DATE %within% test_interval) %>% select(DATETIME, VIX) %>% 
    left_join(model_vix_ret_pred) %>% mutate(VIX_PRED = lag(VIX * exp(MODEL_PRED)))
...
ggplot(vix_plot_data, aes(x = id, y = Price)) + geom_line(aes(color = Type), size = 0.5) + 
    scale_color_manual(values = c("#00AFBB", "#E7B800")) + theme_minimal() + scale_x_continuous(name = "Observation") + 
    ggtitle("LSTM based VIX prediction") + theme(plot.title = element_text(hjust = 0.5))
